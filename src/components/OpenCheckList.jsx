...
const OpenCheckList = () => {
	...
	useEffect(() => refTextArea.current.value = '',[list]);
	...
	return (
		...
		<>
			...
			<article className={`open-note ${conf.theme} ${note.bgColor || conf.bgColor}`}>
				...
				<div className="none" ref={addElementRef}>
					...
					<textarea className="add-el-list" ref={refTextArea} type="text" placeholder="agregar elemento" disabled></textarea>

				</div>

				<ul>
				
					{list.map((el,i) => <div className="list-element" key={i}>

						{(edit && !el.finished) ? <input className="input-list" id={i} value={el.value} onChange={handlerChange}/> : <li className={`${(el.finished) ? 'finished' : ''}`} id={i} onClick={(!edit) ? checkElement : () =>{}}>{el.value}</li>}
